...
async function check({event,pot,setPot,setData,setPoints,setCenter,points,setDisable,setzoomlev,data,city,setMessage}){
    ...
    <>
      <div >
        ...
        className="px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition" 
         onClick={(event)=>check({event,pot,setPot,setData,setPoints,setCenter,points,setDisable,setzoomlev,data,city,setMessage})}>Submit For {pot} {pot===1?"point":"points"}</button>
        <p>{message}</p>
      </div>
    </>
  );
